...
function applyQueryPreset(): void {
  const queryArtist = getQueryString(route.query.artist)
  const queryDuration = getQueryNumber(route.query.duration || route.query.durationMinutes)
  const querySongCount = getQueryNumber(route.query.songs || route.query.songCount)
  const queryFadeSeconds = getQueryNumber(route.query.fade || route.query.fadeSeconds)

  if (queryArtist) {
    artist.value = queryArtist
  }

  if (queryDuration !== null) {
    durationMinutes.value = queryDuration
  }

  if (querySongCount !== null) {
    songCount.value = querySongCount
  }
      ...
      <div class="page-layout">
        ...
        <div class="primary-column">
          <section class="app-card" aria-labelledby="page-title">
            ...
            <AdSlot v-if="shouldShowAds" position="after-preview" />
          </section>
          ...
          <AdSlot v-if="shouldShowAds" position="bottom" />
        </div>

        <div v-if="shouldShowAds" class="sidebar-column">
